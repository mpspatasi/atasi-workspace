...
// LOGIKA SUPERADMIN BERDASARKAN KOLOM 'modul'
...
try {
    ...
    $kpiHariIni = $pdo->query("SELECT IFNULL(SUM(qty), 0) FROM jurnal_transaksi_atasi.db_inventory_in WHERE DATE(tanggal) = CURDATE()")->fetchColumn();
    $kpiBulanIni = $pdo->query("SELECT COUNT(*) FROM jurnal_transaksi_atasi.db_inventory_in WHERE MONTH(tanggal) = MONTH(CURDATE()) AND YEAR(tanggal) = YEAR(CURDATE())")->fetchColumn();
} catch (PDOException $e) {
    $kpiHariIni = 0; 
    $kpiBulanIni = 0;
}
...
<body class="bg-slate-50 font-sans text-slate-800 h-screen flex overflow-hidden" onclick="closeProfileMenu(event)">
    ...
    <main class="flex-1 flex flex-col h-screen min-w-0 overflow-hidden relative bg-slate-50">
        ...
        <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 shadow-sm z-30">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-bold text-slate-800">Stock In</h1>
                    <div class="flex items-center gap-2 text-xs font-medium mt-0.5">
                        <span class="text-[#F59E0B]">ATASI</span><span class="text-slate-400">/</span><span class="text-slate-500">Inventory In</span>
                    </div>
                </div>
            </div>
        ...
        <div class="p-6 flex flex-col flex-1 min-h-0 relative overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5 shrink-0">
                <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Item Masuk Hari Ini</p>
                        <h3 class="text-2xl font-bold text-slate-800 mt-1"><?= number_format($kpiHariIni, 0, ',', '.') ?> <span class="text-sm text-slate-500 font-medium">Pcs</span></h3>
                    </div>
                ...
                <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Transaksi IN Bulan Ini</p>
                        <h3 class="text-2xl font-bold text-blue-600 mt-1"><?= number_format($kpiBulanIni, 0, ',', '.') ?> <span class="text-sm text-slate-500 font-medium">Trx</span></h3>
                    </div>
            ...
            <div class="mb-4 shrink-0 bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                    <div class="relative w-full md:w-56">
                        ...
                        <input type="text" id="globalSearchInput" placeholder="Cari Dokumen..." class="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm focus:border-[#F59E0B] outline-none transition-all focus:ring-1 focus:ring-orange-200">
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
                        <select id="dateFilterPreset" onchange="handleDatePreset(this.value)" class="w-full md:w-44 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:border-[#F59E0B] outline-none bg-white text-slate-600 font-medium">
                            ...
                            <option value="this_quarters">Kuartal Ini</option>
                ...
                <div class="flex items-center gap-2 w-full md:w-auto">
                    ...
                    <button onclick="openModal('addModal')" class="px-4 py-2 bg-[#F59E0B] hover:bg-orange-600 text-white rounded-lg text-xs font-bold shadow-md flex items-center gap-1.5 transition whitespace-nowrap">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> Tambah Stock IN
                    </button>
                </div>
            </div>
                ...
                <table class="w-full border-collapse text-sm whitespace-nowrap min-w-full">
                    <thead class="sticky top-0 z-20 shadow-sm">
                        <tr class="bg-slate-100 text-slate-600 uppercase tracking-wider text-[11px] font-bold">
                            ...
                            <th class="border-b border-r border-slate-200 px-4 py-3 bg-slate-50 text-center">Qty Masuk</th>
                            <th class="border-b border-r border-slate-200 px-4 py-3 bg-slate-50 text-left">Keterangan</th>
    ...
    <div id="addModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4">
        ...
        <form id="addForm" onsubmit="submitAction(event, 'add')" class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative z-10 modal-box transform scale-95 opacity-0">
            <div class="px-6 py-4 border-b flex justify-between items-center bg-slate-50 rounded-t-xl">
                <h3 class="font-bold text-slate-800">Form Barang Masuk (Stock IN)</h3>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">Tanggal Masuk <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal" value="<?= date('Y-m-d') ?>" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B]" required>
                    </div>
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">No. Referensi / PO</label>
                        <input type="text" name="referensi" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B]" placeholder="Cth: PO-2026-05-001">
                    </div>
                </div>
                <div class="space-y-1">
                    <label class="text-[11px] font-bold text-slate-500 uppercase">Pilih Barang <span class="text-red-500">*</span></label>
                    <select name="sku" id="selectBarang" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B] bg-white" required onchange="updateSatuan('add')">
                        ...
                        <?php foreach($list_barang as $brg): ?>
                            <option value="<?= $brg['sku'] ?>" data-satuan="<?= htmlspecialchars($brg['satuan']) ?>">
                                <?= htmlspecialchars($brg['sku']) ?> - <?= htmlspecialchars($brg['nama_barang']) ?> (Stok: <?= $brg['current_stock'] ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">Qty Masuk <span class="text-red-500">*</span></label>
                        <div class="flex">
                            <input type="number" name="qty" min="1" class="w-full border border-r-0 rounded-l-lg p-2.5 text-sm outline-none focus:border-[#F59E0B]" placeholder="0" required>
                    ...
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">Supplier</label>
                        <input type="text" name="supplier" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B]" placeholder="Opsional">
                    </div>
                </div>
                <div class="space-y-1">
                    <label class="text-[11px] font-bold text-slate-500 uppercase">Catatan / Keterangan</label>
                    <textarea name="keterangan" rows="2" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B]" placeholder="Keterangan tambahan..."></textarea>
                </div>
            </div>
            <div class="px-6 py-4 bg-slate-50 flex justify-end gap-3 rounded-b-xl border-t shrink-0">
                ...
                <button type="submit" class="px-6 py-2.5 bg-[#F59E0B] text-white rounded-lg text-xs font-bold hover:bg-orange-600 shadow-md transition">Simpan Stock IN</button>
            </div>
        </form>
    </div>
        ...
        <form id="editForm" onsubmit="submitAction(event, 'edit')" class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative z-10 modal-box transform scale-95 opacity-0">
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">Tanggal Masuk <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal" id="edit_tanggal" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B]" required>
                    </div>
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">No. Referensi / PO</label>
                        <input type="text" name="referensi" id="edit_referensi" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B]">
                    </div>
                </div>
                <div class="space-y-1">
                    <label class="text-[11px] font-bold text-slate-500 uppercase">Barang <span class="text-red-500">*</span></label>
                    <select name="sku" id="edit_sku" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B] bg-white" required onchange="updateSatuan('edit')">
                        <?php foreach($list_barang as $brg): ?>
                            <option value="<?= $brg['sku'] ?>" data-satuan="<?= htmlspecialchars($brg['satuan']) ?>">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">Qty Masuk <span class="text-red-500">*</span></label>
                        <div class="flex">
                            <input type="number" name="qty" id="edit_qty" min="1" class="w-full border border-r-0 rounded-l-lg p-2.5 text-sm outline-none focus:border-[#F59E0B]" required>
                    ...
                    <div class="space-y-1">
                        <label class="text-[11px] font-bold text-slate-500 uppercase">Supplier</label>
                        <input type="text" name="supplier" id="edit_supplier" class="w-full border p-2.5 rounded-lg text-sm outline-none focus:border-[#F59E0B]">
                    </div>
                </div>
                <div class="space-y-1">
            ...
            <div class="px-6 py-4 bg-slate-50 flex justify-end gap-3 rounded-b-xl border-t shrink-0">
                ...
                <button type="submit" class="px-6 py-2.5 bg-[#F59E0B] text-white rounded-lg text-xs font-bold hover:bg-orange-600 shadow-md transition">Update Perubahan</button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        async function loadTableData() {
            ...
            tbody.innerHTML = `<tr><td colspan="8" class="p-12 text-center text-slate-400 font-medium bg-slate-50 animate-pulse">Menarik data transaksi...</td></tr>`;
            
            try {
                const response = await fetch('ajax_inventory_in.php', {
                    method: 'POST',
                ...
                if (res.data && res.data.length > 0) {
                    ...
                    res.data.forEach((row, idx) => {
                        ...
                        html += `<tr class="hover:bg-slate-50 group transition-colors">
                            ...
                            <td class="border-b border-r px-4 py-2 text-center font-bold text-emerald-600">+${row.qty} <span class="text-[10px] font-medium text-slate-500">${row.satuan || ''}</span></td>
                            <td class="border-b border-r px-4 py-2 text-slate-500 text-xs">
                                ${row.supplier ? `<span class="font-bold text-slate-600">${row.supplier}</span><br>` : ''}
                                ${row.keterangan || '-'}
                            </td>
                    ...
                    tbody.innerHTML = `<tr><td colspan="8" class="p-12 text-center text-slate-400 font-medium bg-slate-50">Tidak ada riwayat transaksi.</td></tr>`;
                }
                ...
                tbody.innerHTML = `<tr><td colspan="8" class="p-12 text-center text-red-500 font-medium bg-slate-50">Gagal terhubung ke server.</td></tr>`;
            }
        }
        ...
        async function submitAction(e, actionType) {
            ...
            const titleConf = actionType === 'add' ? 'Simpan Data Masuk?' : 'Update Perubahan?';
            const textConf = actionType === 'add' ? 'Stok utama barang ini akan otomatis bertambah.' : 'Master stok akan otomatis dikalkulasi ulang.';

            Swal.fire({
                title: titleConf, text: textConf, icon: 'question',
                showCancelButton: true, confirmButtonColor: '#F59E0B',
                confirmButtonText: 'Ya, Lanjutkan!', cancelButtonText: 'Batal'
            }).then(async (result) => {
                if (result.isConfirmed) {
                    try {
                        const response = await fetch('proses_inventory_in.php', { method: 'POST', body: formData });
                        const res = await response.json();
        ...
        function fillEditModal(data) {
            ...
            document.getElementById('edit_tanggal').value = data.tanggal.split(' ')[0]; // Ambil YYYY-MM-DD
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                text: "Menghapus ini akan memotong kembali Master Stok sesuai jumlah Qty masuk!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#ef4444', confirmButtonText: 'Ya, Hapus permanen!', cancelButtonText: 'Batal'
            }).then(async (result) => {
                if (result.isConfirmed) {
                    const fd = new FormData(); fd.append('action', 'delete'); fd.append('id', id);
                    const res = await fetch('proses_inventory_in.php', { method: 'POST', body: fd }).then(r => r.json());
                    if (res.status === 'success') {
